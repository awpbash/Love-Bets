...
const sampleMatches = [
  {
      id: 1,
      user1: { name: "Kim Kardashian", profile_pic: "../images/celeb/kim_kardashian.jpg" },
      user2: { name: "Pete Davidson", profile_pic: "../images/celeb/pete_davidson.jpg" },
      ...
      description: "Will Kim Kardashian and Pete Davidson stay together, or is it over? Place your bets!",
      comments: [
  ...
  },
  {
      ...
      description: "Jay-Z and Beyoncé: Break Up or eternal power couple? Bet now!",
      comments: [
          { username: "musicFan", text: "They’re untouchable!" },
          { username: "trendWatcher", text: "Break Up odds are too low to ignore." },
      ],
  },
  {
      id: 3,
      user1: { name: "Blake Lively", profile_pic: "../images/celeb/blake_lively.jpg" },
      user2: { name: "Ryan Reynolds", profile_pic: "../images/celeb/ryan_reynolds.jpg" },
      ...
      description: "Blake Lively and Ryan Reynolds: Can they continue their perfect relationship? Bet now!",
      comments: [
  ...
  },
  {
      ...
      user2: { name: "Sophie Turner", profile_pic: "../images/celeb/sophie_turner.jpg" },
      ...
      description: "Joe Jonas and Sophie Turner: Is this the end of their story? Place your bets!",
      comments: [
...
const BettingPage = () => {
    ...
    const fetchUserMoney = async () => {
        try {
            const response = await fetch(`http://127.0.0.1:5000/users/get/${userEmail}`, {
                method: "GET",
                headers: {
    ...
    const [selectedMatch, setSelectedMatch] = useState({
        user1: { name: "" },
        user2: { name: "" },
        oddsHistory: [{ breakUp: true, getTogether: false, time: "" }],
        bettingVolume: { breakUp: 12, getTogether: 15 },
        comments: [],
    });
    ...
    const handleBetPopup = async (match) => {
        try {
            setSelectedMatch(match); // Select the match for the popup
            setShowPopup(true); // Open the popup
    
            // Optional: Fetch the current state of the bet from the backend if needed
            const response = await fetch(`http://127.0.0.1:5000/bets/get/${match.id}`, {
    ...
    const handlePlaceBet = async (option) => {
        ...
        try {
            ...
            const fetchResponse = await fetch(`http://127.0.0.1:5000/bets/get/${selectedMatch.id}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                },
            ...
            if (option === "Get Together") {
                const lastValue = updatedCumulativeFor[updatedCumulativeFor.length - 1] || 0;
                const newValue = lastValue + parseFloat(betAmount);
                updatedCumulativeFor.push(newValue);
            } else if (option === "Break Up") {
                const lastValue = updatedCumulativeAgainst[updatedCumulativeAgainst.length - 1] || 0;
                const newValue = lastValue + parseFloat(betAmount);
                updatedCumulativeAgainst.push(newValue);
            }
    
            // Compute updated odds
            const cumulativeAgainstLast = updatedCumulativeAgainst[updatedCumulativeAgainst.length - 1];
            const cumulativeForLast = updatedCumulativeFor[updatedCumulativeFor.length - 1];
            ...
            // Update oddsHistory
            const newOddsHistory = [...selectedMatch.oddsHistory, {
            ...
            const updateResponse = await fetch(`http://127.0.0.1:5000/bets/update/${selectedMatch.id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                },
    ...
    const fetchFriendMatches = async () => {
      try {
          ...
          const friendsResponse = await fetch(`http://127.0.0.1:5000/friends/list_bets/${userId}`, {
              method: "GET",
              headers: {
                  "Content-Type": "application/json",
              },
          ...
          if (!friendsResponse.ok) {
              console.error("Failed to fetch friends list:", await friendsResponse.json());
              return;
          }
          ...
          const betsResponse = await fetch("http://127.0.0.1:5000/bets/friends_bets", {
              method: "POST",
              headers: {
                  "Content-Type": "application/json",
              },
          ...
          if (!betsResponse.ok) {
              console.error("Failed to fetch friend matches:", await betsResponse.json());
              return;
          }
          ...
          const transformedMatches = betsData.bets.map((bet) => {
              ...
              const cumulativeAgainstLast = cumulativeAgainst[cumulativeAgainst.length - 1];
              const cumulativeForLast = cumulativeFor[cumulativeFor.length - 1];
  
              const breakUp = Math.round(
                  (cumulativeAgainstLast / (cumulativeAgainstLast + cumulativeForLast)) * 100
              );
              const getTogether = 100 - breakUp;
              ...
              return {
                  ...
                  comments: [
                    ...
                    { username: "analystPro", text: "Get Together seems like a safe bet." },
  ...
  const getChartData = (history) => ({
    labels: history?.map((entry) => entry.time) || [], // Fallback to an empty array if history is undefined
    datasets: [
        {
            label: "Break Up",
            data: history?.map((entry) => entry.breakUp) || [], // Fallback to an empty array
            borderColor: "#ff6666",
        ...
        {
            label: "Get Together",
            data: history?.map((entry) => entry.getTogether) || [], // Fallback to an empty array
            borderColor: "#0073b1",
  ...
  const page1 = (
    ...
    >
        ...
        <div className={styles.scrollable}>
            {activeTab === "top" ? renderCards(sampleMatches) : renderCards(friendMatches)}
        </div>
    </div>
);
    ...
    >
      <div className={styles.popupContent}>
        ...
        <div className={styles.heartAnimation}>
          <img
            src={selectedMatch.user1.profile_pic|| "/images/quackers.jpg"}
            alt={selectedMatch.user1.name}
            className={styles.userAvatarHeart}
          />
          <div className={styles.heart}></div>
          <img
            src={selectedMatch.user2.profile_pic|| "/images/quackers.jpg"}
            alt={selectedMatch.user2.name}
            className={styles.userAvatarHeart}
          />
        </div>
        ...
        >
          ...
          <div className={`${styles.logo} logo`}>
            Place your BETS for {selectedMatch.user1.name} &{" "}
            {selectedMatch.user2.name}
          </div>
        </div>
  
        {/* Match Description */}
        <p className={styles.description}>{selectedMatch.description}</p>
  
        {/* Charts Section */}
        <div className={styles.charts}>
          <h3>Odds Over Time</h3>
          <Line data={getChartData(selectedMatch.oddsHistory)} />
          <h3>Betting Volume</h3>
          <Bar data={getBarChartData(selectedMatch.bettingVolume)} />
        </div>
